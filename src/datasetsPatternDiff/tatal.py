...
class FashionPatternDataset(data.Dataset):
    def __init__(self,
                 dataroot_path: str,
                 phase: str,
                 tokenizer,
                 radius=5,
                 caption_folder='caption_upper.json',
                 coarse_caption_folder='caption_upper.json',
                 sketch_threshold_range: Tuple[int, int] = (20, 127),
                 order: str = 'paired',
                 outputlist: Tuple[str] = ('c_name', 'im_name', 'image', 'im_cloth', 'shape',"im_parse",
                                           'parse_array', 'im_mask', 'inpaint_mask', 'parse_mask_total',
                                           'captions',
                                           'original_captions'),
                 category: Tuple[str] = ('dresses', 'upper_body', 'lower_body'),
                 size: Tuple[int, int] = (512, 384),
                 dataset_c: Tuple[str] = ('dresscode','vitonhd','streettryon'),
                 ):

        super(FashionPatternDataset, self).__init__()
        ...
        self.dataset_paths = dataroot_path
        ...
        possible_outputs = ['c_name', 'im_name', 'cloth', 'image', 'im_cloth', 'shape', 'im_head', 'im_pose',
                             'parse_array', 'dense_labels', 'dense_uv', 'skeleton',"im_parse",
                            'im_mask', 'inpaint_mask', 'parse_mask_total', 'cloth_sketch', 'captions',
                            'original_captions', 'category', 'hands', 'parse_head_2']

        assert all(x in possible_outputs for x in outputlist)
        self.captions_dict_total = {}
        if phase == 'train': 
            for ca in self.dataset_c:
        # Load Captions
                if ca == 'dresscode':
                    with open(os.path.join(os.path.join(self.dataset_paths,ca),'dresscode.json')) as f:
                        self.captions_dict = json.load(f)
                    self.captions_dict = {k: v for k, v in self.captions_dict.items()}
                    self.captions_dict_total['dresscode'] = self.captions_dict
                elif ca == 'vitonhd':
                    with open(os.path.join(os.path.join(self.dataset_paths,ca),'vitonhd.json')) as f:
                        self.captions_dict = json.load(f)
                    self.captions_dict = {k: v for k, v in self.captions_dict.items()}
            ...
            for ca in self.dataset_c:
                if ca == 'dresscode':
                    ...
                    with open(os.path.join(self.dataset_paths,ca,'vitonhd.json')) as f:
                        self.captions_dict = json.load(f)
                    self.captions_dict = {k: v for k, v in self.captions_dict.items()}
                    self.captions_dict_total['vitonhd'] = self.captions_dict
                elif ca == 'streettryon':
        ...
        for ca in self.dataset_c:
            if ca == 'dresscode':
                dataroot = os.path.join(self.dataset_paths,ca)
                if phase == 'train':
                        filename = os.path.join(dataroot,'train_dresscode.txt')
                else:
                        filename = os.path.join(dataroot,'test_dresscode.txt')
                ...
                with open(filename, 'r') as f:
                    for line in f.readlines():
                        if phase == 'train':
                            ...
                            if order == 'paired':
                                im_name, _ = line.split(' ')
                                c_name = im_name
                            else:
                                im_name, c_name = line.split(' ')
                                c_name = c_name.strip()

                        im_names.append(im_name.replace('1.jpg','0.jpg'))
                        c_names.append(c_name.replace('1.jpg','0.jpg'))
                        dataroot_names.append(dataroot)
                        data_category.append(ca)
        # vitonhd
            elif ca == 'vitonhd':
                dataroot = os.path.join(self.dataset_paths,ca)
                if phase == 'train':
                    filename = os.path.join(dataroot,'train_vitonhd.txt')
                else:
                    filename = os.path.join(dataroot,'test_vitonhd.txt')
                with open(filename, 'r') as f:
                    for line in f.readlines():
                        if phase == 'train':
                            im_name = line.strip()
        ...
        # streettryon
                ...
                dataroot = os.path.join(self.dataset_paths,ca)
                if phase == 'train':
                        filename = os.path.join(dataroot,'train_streettryon.txt')
                else:
                        filename = os.path.join(dataroot,'test_streettryon.txt')
    ...
    def __getitem__(self, index):
        ...
        data_category = self.data_category[index]

        if data_category == 'dresscode':
            ...
            if "captions" in self.outputlist or "original_captions" in self.outputlist:
                captions = self.captions_dict_total[data_category][c_name.replace('.jpg','')]
            ...
            #     result[k] = vars()[k]
            result['im_parse'] = im_parse
            result['image'] = image
            result['inpaint_mask'] = inpaint_mask
            result['captions'] = captions
            result['original_captions'] = original_captions
            result['c_name'] = c_name
            result['im_name'] = im_name
            result['parse_array'] = parse_array
            #result['c_name'] = c_name

            
            im_name_pose = im_name.replace('_0.jpg', '_5.jpg')
            pose_image = Image.open(os.path.join(dataroot ,  'skeletons' , im_name_pose)).resize((384,512))
            pose_image = torch.from_numpy(np.array(pose_image).transpose(2,0,1) / 255)
            result['openpose'] = pose_image
            
            texture_image = c_name.replace('_0.jpg', '_1.jpg')
            texture = Image.open(os.path.join(dataroot , 'texture' , texture_image)).convert("RGB").resize((224,224))
            texture = self.transform(texture)
            result['texture'] = texture
            
            pattern_image = c_name.replace('_0.jpg', '_1.jpg')
            source_pattern = Image.open(os.path.join(dataroot , 'pattern' , pattern_image)).convert("RGB")
            pattern = self.transform(source_pattern.resize((224,224)))
            source_pattern = torch.from_numpy(np.array(source_pattern).transpose(2,0,1)  / 255.)
            source_pattern = 2 * source_pattern - 1
            result['pattern_source'] = source_pattern
            ...
            im_name_pose = im_name.replace('.jpg','_rendered.png')
            pose_image = Image.open(os.path.join(dataroot, self.phase, 'openpose_img', im_name_pose)).resize((384,512))
            pose_image = torch.from_numpy(np.array(pose_image).transpose(2,0,1)  / 255.)
            result['openpose'] = pose_image
            
            texture_image = c_name
            texture = Image.open(os.path.join(dataroot, self.phase, 'texture', texture_image)).convert("RGB").resize((224,224))
            texture = self.transform(texture)
            result['texture'] = texture
            
            pattern_image = c_name
            source_pattern = Image.open(os.path.join(dataroot, self.phase, 'pattern', pattern_image)).convert("RGB")
            pattern = self.transform(source_pattern.resize((224,224)))
            source_pattern = torch.from_numpy(np.array(source_pattern).transpose(2,0,1)  / 255.)
            source_pattern = 2 * source_pattern - 1
            result['pattern_source'] = source_pattern
            ...
            if "im_pose" in self.outputlist or "parser_mask" in self.outputlist or "im_mask" in self.outputlist or "parse_mask_total" in self.outputlist or "parse_array" in self.outputlist or "pose_map" in self.outputlist or "parse_array" in self.outputlist or "shape" in self.outputlist or "im_head" in self.outputlist:
                # Label Map
                parse_name = im_name.replace('jpg', 'png')
                im_parse = Image.open(os.path.join(dataroot , self.phase , 'segm_atr_labels' , parse_name))
                im_parse = im_parse.resize((self.width, self.height), Image.NEAREST)
                parse_array = np.array(im_parse)
            ...
            im_name_pose = im_name.replace('.jpg', '_keypoints.jpg')
            pose_image = Image.open(os.path.join(dataroot ,self.phase ,   'keypoint_images' , im_name_pose)).resize((384,512))
            pose_image = torch.from_numpy(np.array(pose_image).transpose(2,0,1) / 255)
            result['openpose'] = pose_image
